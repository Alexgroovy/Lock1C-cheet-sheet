...
#### 1. Метод `НаборЗаписей.Прочитать()` всегда устанавливает __управляемую разделяемую__ блокировку для следующих объектов:
...
#### 3. Платформа НЕ устанавливает _управляемые_ блокировки при объектном _чтении_:
 ...
 - исключение составляет НаборЗаписей.Прочитать()
...
>
> ### 2. Иногда чтение происходит в неявной транзакции. (УСТАРЕЛО для 8.3.7+)
...
К счастью с 8.3.8+ от неявной транзакции при чтении объекта отказались.

_Следующая цитата относится к старым платформам_:
> 
> Поэтому, если в СУБД предполагалось установка блокировки (автоматический режим управления блокировками или, например платформа в режиме совместимости 8.2.16 и MS SQL Server) - в формах списка документов могут возинкать конфликты блокировок, если в событиях ПриПолученииСтроки() и в ПриВыводеСтроки() есть запросы через объектную модель, а время проведения документов превышает установленный для информационной базы лимит времени ожидания на блокировке (по умолчанию 20 сек).
> 
...
Пессимистическая блокировка храниться в [сервисе блокировок](https://github.com/kuzyara/Lock1C-cheet-sheet/blob/master/images/lock-services-screen.png) DataEditLockService кластера 1С. Увидеть блокировку конкретного объекта можно в консоли кластера в разделе Кластеры -> Информационные базы -> Блокировки: 
...
Процедура ОбработкаПроведения()
    НаборЗаписей.Прочитать(); // <- тут наложится неявная разделяемая управляемая блокировка (до конца транзакции!)
````
при записи:
```bsl
Объект.Записать(); // <- тут наложится неявная исключительная управляемая блокировка
````
--------------------------------------------------
